...
def plot_envelope(data, nfft, fs, lifter, dimension):
    ...
    log_data = 20 * np.log10(np.abs(spec) + np.finfo(np.float64).eps)
    ...
    plt.figure()
    plt.plot(freq[:nfft // 2], log_data[:nfft // 2], label='Spectrum')
    plt.plot(freq[:nfft // 2], cep_data[:nfft // 2], label='Cepstrum Envelope')
    plt.plot(freq[:nfft // 2], lpc_data[:nfft // 2], label='LPC')
