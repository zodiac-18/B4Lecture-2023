...
def main():
    ...
    parser.add_argument(
        "-y", "--ylim", help="upper limit of y for the graph", default=3000, type=int
    )
    parser.add_argument(
        "-t",
        "--tap",
    ...
    s.draw_spectrogram(data, ax=ax1, framesize=framesize, time=time, y_limit=y_limit, overlap=overlap, samplerate=samplerate)
    # Create a time axis
    ...
    s.draw_spectrogram(data, ax=ax2, framesize=framesize, time=time, y_limit=y_limit, overlap=overlap, samplerate=samplerate)
