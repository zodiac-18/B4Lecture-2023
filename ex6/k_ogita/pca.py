...
def main():
    ...
    if dim == 2:
        ...
        ax1.axline((0, 0), w[0], color = "red", label=f"Contribution rate:{con_rate[0]:.3f}")
        ax1.axline((0, 0), w[1], color = "orange",label=f"Contribution rate:{con_rate[1]:.3f}")
        ax1.set_xlabel("x1")
        ax1.set_ylabel("x2")
        ax1.set_title("")
        ...
        def animation_init():
            ...
            ax1.plot(x, slope_xy[0]*x, slope_yz[0]*x, color="red", label=f"Contribution rate:{con_rate[0]:.3f}")
            ax1.plot(x, slope_xy[1]*x, slope_yz[1]*x, color = "orange",label=f"Contribution rate:{con_rate[1]:.3f}")
            ax1.plot(x, slope_xy[2]*x, slope_yz[2]*x, color = "yellow",label=f"Contribution rate:{con_rate[2]:.3f}")
        ...
        def animate(i):
            ...
            return fig1,
        ani = animation(fig1, func=animate, init_func=animation_init, frames=np.arange(60), interval=100, blit=True, repeat=False)
        ax1.set_title(f"Basis of principal components of {file_name}")
        ax1.legend()
        ...
        ax2.scatter(comp_data[:, 0], comp_data[:, 1], c="blue", label=f"compressed data")
        ax2.set_xlabel("x")
        ax2.set_ylabel("y")
        ...
        ax1.axvline(x=order, color="orange", label=f"order of when the cumulative contribution reaches 90% (order={order})")
