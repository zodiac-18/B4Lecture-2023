...
def load_file(filename):
    ...
    total_time = len(y)/sr
    time_array = np.arange(0, total_time, 1/sr)
    return sr, y, time_array


def stft(data, win_size, overlap):
    spec = []                        # スペクトログラム保持用
    win_func = np.hamming(win_size)  # 窓関数の用意
    step = win_size-overlap

    for i in range(int((data.shape[0]-win_size)/step)):
...
def main():
    # 音声の読込
    sample_rate, data, time_array = load_file("sample.mp3")
    ...
    # 逆変換で得た波形の表示
    librosa.display.waveshow(new_data, sr=sample_rate, ax=ax[2])
